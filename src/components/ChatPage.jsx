  ...
  useEffect(()=>{
    const fetchMessages = async () => {
      ...
      try {
        ...
        setTimeout(()=>{chatboxRef.current.scrollTop = chatboxRef.current.scrollHeight;},10)
        setLoading(false)
      } catch (error) {
  ...
  return (
    <div className="chatbot-container">
      <div className="chatbox" ref={chatboxRef}>
        <ul>
          {!loading && chatHistory.map((message, index) => (
            <li key={index} className={`chat ${message.role}`}>
              {message.role === "assistant" ? (
                <span className="material-symbols-outlined avatar-assistant">smart_toy</span>
              ) : (
                <span className="material-symbols-outlined avatar-user">person</span>
              )}
              <p>{message.content}</p>
            </li>
          ))}
